...
const CreateJam = () => {
    const [songsInJam,setSongsInJam]=useState<object[]>([]);
   ...
   //   }
     useEffect(()=>{
      // console.log("songs in jam updated",songsInJam);
      },[songsInJam])
   ...
   const fetchSongs = async ({ queryKey }: { queryKey: [string, string] }) => {
      ...
      // console.log(response.data.data,"song response")
      ...
      const addToJamList=(track)=>{
         // console.log("adding to jam list",track);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-8">
        <div className="max-w-2xl mx-auto">
            ...
            <form className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                <div>jam name:</div>
            </form>
        </div>
    </div>
